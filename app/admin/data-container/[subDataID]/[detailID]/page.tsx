...
export default function Page({
  ...
  params: { subDataID: string; detailID: string };
}) {
  ...
  useEffect(() => {
    ...
    fetchAnnouncement();
  }, [params.detailID, params.subDataID]);
  ...
  const saveToFirestore = async () => {
    ...
    try {
      ...
      await updateDoc(
        doc(db, "reference", params.subDataID, "sub", params.detailID),
        {
          title: title,
          content: text,
          author: "관리자",
  ...
  const handleUploadFile = async (file: any) => {
    const storageReference = storageRef(
      storage,
      `uploads/reference/${params.subDataID}/${params.detailID}/${file?.name}`
    );
