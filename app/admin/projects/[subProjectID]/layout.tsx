...
export default function Layout({
  ...
  params: { subProjectID: string };
}) {
  const [clickedIndex, setClickedIndex] = useState<number>(-1);
  const [subProjects, setSubProjects] = useState<any[]>([]);
  useEffect(() => {
    getDocs(collection(db, "projects", params.subProjectID, "sub")).then(
      (querySnapshot) => {
    ...
    <div className="flex w-full min-h-screen m-0">
      <div className="flex w-full h-full">
        <div className="w-1/5 h-full border-r border-gray-300 p-4 flex flex-col mb-4">
          <Link
            href={`/admin/projects/${params.subProjectID}`}
          ...
          <div className=" flex flex-col mb-4">
            {subProjects.map((project: any, index: number) => (
              <Link
                ...
                href={`/admin/projects/${params.subProjectID}/${project.id}`}
                key={index}
